...
const AddProject = () => {
  const currentStep = useSelector((state: RootState) => state.newPublication.step);
  ...
  return (
	<StyledBackground>
		<StyledContainer>
			<Typography variant='h1' style={{fontWeight: 'bold'}}>New Project</Typography>
			<Typography variant='h4'>step {currentStep}/5</Typography>
			<StepContainer>
				{renderStepComponent()}
			</StepContainer>
			<div>
				<StyledButtonBlack onClick={() => {if(currentStep > 1) dispatch(decreaseStep())}}>BACK</StyledButtonBlack>
				<StyledButtonGreen onClick={() => {if(currentStep < 5) dispatch(increaseStep())}}>NEXT</StyledButtonGreen>
			</div>
		</StyledContainer>
	</StyledBackground>
  );
