...
export default function StepOne(props: any) {
    ...
    const [imageValidationTriggered, setImageValidationTriggered] = useState(false)
    ...
    useEffect(() => {
        ...
        if (nameValidationTriggered && descValidationTriggered) {
            (imagePickerError || nameInputError || descInputError) ? props.setNextButtonEnabled(false) : props.setNextButtonEnabled(true)
        }
    ...
    return (
        <div className="grid-container">
            <div className="left-column">
                <InputLabel error={imagePickerError} typographyVariant='h3'>{t('StepOne.projectPhoto')}</InputLabel>
                <ImagePicker error={imagePickerError} onClick={() => handleImageValidation()} />
            </div>
            <div className="right-column">
                <InputLabel error={nameInputError} typographyVariant='h3'>{t('StepOne.projectName')}</InputLabel>
                <TextField className="name-input-field" placeholder={t('StepOne.projectNameDetails')} error={nameInputError} onChange={(e) => handleNameValidation(e.target.value)} />
                <InputLabel error={descInputError} typographyVariant='h3'>{t('StepOne.projectDescription')}</InputLabel>
                <TextArea className="desc-input-field" placeholder={t('StepOne.projectDescriptionDetails')} error={descInputError} onChange={(e) => handleDescValidation(e.target.value)} />
            </div>
        </div>
    )
}
