...
import { selectCurrentDirectory, selectHasDirectoryBeenSaved, setHasDirectoryBeenSaved, updateCurrentDirectory } from '../../../../shared/slices/directoriesSlice';
...
const Files = () => {
  ...
  const directoriesHistory = JSON.parse(localStorage.getItem("directoriesHistory") || "[]");

  const onProceedClicked = () => {
    if (!currentDirectory.path || currentDirectory.path == "-") return;

    if (Array.isArray(directoriesHistory)) {
      const duplicateIndex = directoriesHistory.findIndex(item => item.path === currentDirectory.path);
      if (duplicateIndex != -1) {
        directoriesHistory.splice(duplicateIndex, 1);
      }

      if (directoriesHistory.push(currentDirectory) > 3) directoriesHistory.shift();
  ...
  const onOthersClicked = () => {
    dialog.showOpenDialog({ properties: ['openDirectory'] }).then((data: any) => {
      ...
      {!hasDirectoryBeenSaved ?
        <div className="files__leftSection">
          ...
          <div className="files__lastUsedList">
            ...
            {directoriesHistory && Array.isArray(directoriesHistory) && directoriesHistory.length > 0 ? 
            directoriesHistory.map((item: { path: string; date: string; }) => <Directory path={item.path} lastUsed={new Date(item.date)} onClick={onDirectoryClicked(item.path)}/>)
            : <div className="files__noDirectories">No recent directories</div>
            }
          </div>
          <button className="files__othersButton" onClick={onOthersClicked}>Other</button>
          <div className="files__selected">{currentDirectory.path}</div>
          <button className={!currentDirectory.path || currentDirectory.path == "-" ? "files__proceedButton_disabled" : "files__proceedButton"} onClick={onProceedClicked}>Proceed</button>
        </div>
        :
      ...
      <div className="files__rightSection">
        {hasDirectoryBeenSaved && 
          <div className="files__rightContent">
            ...
            <div className="files__right_subtitle">Click <img src={add_circle_icon}/> to create new project</div>
          </div>
        }
      </div>
